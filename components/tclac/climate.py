...
# Validate interface configuration and set default values
    ...
    if CONF_VISUAL in config:
        ...
        if CONF_MIN_TEMPERATURE in visual_config:
            ...
            if min_temp < TCLAC_MIN_TEMPERATURE:
                raise cv.Invalid(f"Specified interface minimum temperature of {min_temp} is below allowed {TCLAC_MIN_TEMPERATURE} for this AC")
        else:
        ...
        if CONF_MAX_TEMPERATURE in visual_config:
            ...
            if max_temp > TCLAC_MAX_TEMPERATURE:
                raise cv.Invalid(f"Specified interface maximum temperature of {max_temp} is above allowed {TCLAC_MAX_TEMPERATURE} for this AC")
        else:
        ...
        if CONF_TEMPERATURE_STEP in visual_config:
            ...
            if ((int)(temp_step * 2)) / 2 != temp_step:
                raise cv.Invalid(f"Specified temperature step of {temp_step} is invalid, must be a multiple of 1")
        else:
...
# Validate component configuration and set default values
...
# Register module communication LEDs ON/OFF actions
@automation.register_action(
...
# Register force mode state ON/OFF actions
@automation.register_action(
...
# Register vertical louver fixed position action
@automation.register_action(
...
# Register horizontal louvers fixed position action
@automation.register_action(
...
# Register vertical louver swing direction action
@automation.register_action(
...
# Register horizontal louvers swing direction action
@automation.register_action(
...
# Add configuration generator to code
def to_code(config):
